    ...
    getTotalInfo(lat, long, date, time) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                console.log(locData);
                let dust = yield DustInfoModel.find({ locName: locData.location }, { pm10Value: 1, pm25Value: 1, dataTime: 1, _id: 0 }).sort({ dataTime: -1 }).limit(1);
                predict = predict.concat(dust);
                predict = predict.concat(locData);
                return predict;
            }
            catch (error) {
                console.error(error);
